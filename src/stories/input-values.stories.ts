...
export const RawValuesDemo = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    const dateRaw = canvasElement.querySelector('#date-raw') as HTMLElement;
    const phoneRaw = canvasElement.querySelector('#phone-raw') as HTMLElement;
    const ipRaw = canvasElement.querySelector('#ip-raw') as HTMLElement;
    const formResult = canvasElement.querySelector(
      '#form-result',
    ) as HTMLElement;
    ...
    const ipInput = canvasElement.querySelector('#auto-ip') as HTMLInputElement;
    ...
    const form = canvasElement.querySelector('#test-form') as HTMLFormElement;
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      const formData = new FormData(form);
      formResult.textContent = formData.get('phone') as string;
    });
  },
};
